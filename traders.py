...
def _async_openai(base_url: str, api_key: str | None) -> AsyncOpenAI | None:
    if not api_key:
        return None
    return AsyncOpenAI(base_url=base_url, api_key=api_key)


openrouter_client = _async_openai(OPENROUTER_BASE_URL, openrouter_api_key)
deepseek_client = _async_openai(DEEPSEEK_BASE_URL, deepseek_api_key)
grok_client = _async_openai(GROK_BASE_URL, grok_api_key)
gemini_client = _async_openai(GEMINI_BASE_URL, google_api_key)


def get_model(model_name: str):
    if "/" in model_name:
        if not openrouter_client:
            raise ValueError("OPENROUTER_API_KEY is required for OpenRouter model names containing '/'")
